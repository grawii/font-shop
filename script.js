...
function createCardHtml(p, catName) { 
    ...
    let imgContent = Array.isArray(p.img) ? 
        `<div class="snap-slider card-snap-slider h-full w-full">${p.img.map(u => `<div class="snap-item w-full h-full shrink-0"><img src="${u}" class="w-full h-full object-cover"></div>`).join('')}</div>` : 
        `<img src="${p.img}" class="w-full h-full object-cover">`; 
    
    return `
    <div class="item-card bg-white p-3.5 border-2 border-pinky-light shadow-sm transition-all ${isSel}" data-id="${p.id}" onclick="toggleItem('${p.id}','${catName}','${p.name}',${p.price},${p.isPromo || false})">
        ${badgeHtml}
        <div class="rounded-2xl aspect-square mb-3 overflow-hidden relative">${imgContent}</div>
        <h4 class="font-bold text-[14px] text-brown line-clamp-1 font-mitr">${p.name}</h4>
        <p class="text-[9px] text-brown-light mt-0.5 mb-2 line-clamp-2 leading-tight">${p.detail || ''}</p>
        <div class="flex items-center gap-2 mt-1">
            ${p.oldPrice ? `<span class="text-[11px] line-through text-brown-light">฿${p.oldPrice}</span>` : ''}
            <span class="bg-pinky-dark text-white text-[12px] px-2 py-0.5 rounded-lg font-bold">฿${p.price}</span>
        </div>
    </div>`; 
}
...
function viewSubCategory(cId, sT) { 
    ...
    document.getElementById('fullPageCategory').style.display = 'block'; 
    lockScroll(true); 
    lucide.createIcons(); 
}
...
function renderHome() { 
    let recs = []; 
    categories.forEach(c => c.products.forEach(p => { if(p.isRecommend) recs.push({p, cName: c.name}); }));
    document.getElementById('home-recommend').innerHTML = recs.slice(0, 4).map(item => createCardHtml(item.p, item.cName)).join('');
    if(recs.length > 4) document.getElementById('recommend-more-btn').classList.remove('hidden'); 
    else document.getElementById('recommend-more-btn').classList.add('hidden');
    lucide.createIcons();
}
...
function checkout() { 
    ...
    alert("คัดลอกรายการสั่งซื้อแล้ว! กำลังไปที่หน้า LINE..."); 
...
function openSummary() { 
    const r = calculateTotal(); 
    let h = ''; 
    cart.forEach(i => h += `<div class="flex justify-between text-sm"><span>• ${i.name}</span><b>฿${i.price}</b></div>`); 
    document.getElementById('modalItemsList').innerHTML = h; 
    let pT = (r.discAM > 0 ? `✨ ส่วนลด AM: -฿${r.discAM}<br>` : "") + (r.discPRN > 0 ? `✨ ส่วนลด PRN (1แถม1): -฿${r.discPRN}<br>` : "");
...
function initAutoScroll(id) { 
    ...
    setInterval(() => { 
        if (s.scrollLeft + s.offsetWidth >= s.scrollWidth - 10) s.scrollTo({ left: 0, behavior: 'smooth' }); 
        else s.scrollBy({ left: s.offsetWidth, behavior: 'smooth' }); 
    }, 5000); 
}

function initCardAutoScroll() { 
    setInterval(() => { 
        document.querySelectorAll('.card-snap-slider').forEach(s => { 
            if (s.scrollLeft + s.offsetWidth >= s.scrollWidth - 10) s.scrollTo({ left: 0, behavior: 'smooth' }); 
            else s.scrollBy({ left: s.offsetWidth, behavior: 'smooth' }); 
        }); 
    }, 4000); 
    ...
    if(P) { 
        ['⭔','✦','★','♥','🍇'].forEach(e => { 
            for (let j = 0; j < 6; j++) { 
                ...
                d.className = 'pt'; d.textContent = e; 
                d.style.cssText = `left:${Math.random()*100}vw;font-size:${14+Math.random()*10}px;color:#D1C7F0;animation-duration:${10+Math.random()*15}s;animation-delay:${-Math.random()*20}s;`; 
                P.appendChild(d); 
            } 
        }); 
    }
    lucide.createIcons(); 
    initAutoScroll('review-slider'); 
    initCardAutoScroll();
});
