...
// แก้ไขฟังก์ชันเปิด-ปิดเมนูให้ใช้คลาส active
...
function viewSubCategory(cId, sT) { 
    ...
    document.getElementById('fullPageCategory').style.display = 'block'; lockScroll(true); lucide.createIcons(); 
}
...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    if(P) { ['⭔','✦','★','♥','🍇'].forEach(e => { for (let j = 0; j < 6; j++) { const d = document.createElement('div'); d.className = 'pt'; d.textContent = e; d.style.cssText = `left:${Math.random()*100}vw;font-size:${14+Math.random()*10}px;color:#D1C7F0;animation-duration:${10+Math.random()*15}s;animation-delay:${-Math.random()*20}s;`; P.appendChild(d); } }); }
    lucide.createIcons(); 
});
